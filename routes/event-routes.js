...
router.get("/:pageNo?", (req, res) => {
  ...
  //find totoal documents/ show all events
  let totalDocs = 0;
...
//Creating a new event/ save event to db
...
router.delete("/delete/:id", isAuthenticated, (req, res) => {
  ...
  Event.deleteOne(query, (err) => {
    if (!err) {
      res.status(200).json("Poistetu");
    } else {
      res.status(404).json("Virhe tapahtui, tapahtuma ei poistettu.");
    }
  });
});
